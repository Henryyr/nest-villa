...
import { WishlistWithProperty } from './interfaces/wishlist-with-property.type';
...
export class WishlistRepository implements IWishlistRepository {
  ...
  async getWishlist(userId: string, options: FindAllOptions = new FindAllOptions()) {
  ...
  async findAll(options: FindAllOptions = new FindAllOptions()): Promise<{ data: WishlistWithProperty[]; total: number; page: number; limit: number }> {
    const { search, page = 1, limit = 10 } = options;
    const where: Prisma.WishlistWhereInput = {};
    if (search) {
      where.property = {
