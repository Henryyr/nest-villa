...
import { CacheService } from 'src/jobs/cache.service';
import { CachedUser } from 'src/common/interfaces/redis.interface';
import { SessionService } from 'src/jobs/session.service';
import { TokenService } from 'src/jobs/token.service';
import { RateLimitService } from 'src/jobs/rate-limit.service';
...
export class AuthService {
  ...
  async login(payload: LoginAuthDto, ipAddress?: string): Promise<AuthResponseDto> {
    try {
      ...
      // Cache user data
      await this.cacheService.cacheUser(user.id, user as unknown as CachedUser);
  ...
  async getProfile(user: JwtPayload): Promise<UserProfileDto> {
    try {
      ...
      // Cache user profile
      await this.cacheService.cacheUser(user.sub, profile as unknown as CachedUser);
